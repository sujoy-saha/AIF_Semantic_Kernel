...
INCIDENT_MANAGER = "Louise-Incident-Manager"
...
DEVOPS_ASSISTANT = "Tom-DevOps-Assistant"
...
class AIF_SemanticKernelMultiAgent(AIF_SemanticKernelAgent):    
    # Load the data files
    async def load_data(self):
        try:
            ...
            self.file_path = script_dir / "logs"
            shutil.copytree(src_path, self.file_path, dirs_exist_ok=True)                
    ...
    async def create_agents(self):
        try:            
            ...
            log_file_plugin = LogFilePlugin()
            print(INCIDENT_MANAGER)
            self.agent_incident = await self.create_agent(INCIDENT_MANAGER,INCIDENT_MANAGER_INSTRUCTIONS,log_file_plugin)
            # Create the devops agent on the Azure AI agent service
            # Create a Semantic Kernel agent for the devops Azure AI agent
            dev_ops_plugin = DevopsPlugin()
            print(DEVOPS_ASSISTANT)
            self.agent_devops = await self.create_agent(DEVOPS_ASSISTANT,DEVOPS_ASSISTANT_INSTRUCTIONS,dev_ops_plugin)            
        except Exception as ex:
                print(ex)
    ...
    async def process_log_files(self):
        try:
            ...
            for filename in os.listdir(self.file_path):
                logfile_msg = ChatMessageContent(role=AuthorRole.USER, content=f"USER > {self.file_path}/{filename}")
                await asyncio.sleep(30) # Wait to reduce TPM
                print(f"\nReady to process log file: {filename}\n")
                # Append the current log file to the chat
                await self.chat.add_chat_message(logfile_msg)                
                try:                    
    ...
    # execute activities requested for the agent.
    async def execute_task(self):
        try:
            # Add the agents to a group chat with a custom termination and selection strategy            
            self.chat = AgentGroupChat(
                agents=[self.agent_incident, self.agent_devops],
                termination_strategy=ApprovalTerminationStrategy(
                    agents=[self.agent_incident], 
                    maximum_iterations=10, 
                    automatic_reset=True
                ),
                selection_strategy=SelectionStrategy(agents=[self.agent_incident,self.agent_devops]),      
            )
            await self.process_log_files()             
        except Exception as ex:
            print(ex)
    
    # Cleanup: function to delete agents - incident manager and devops assistant.        
    async def delete_agents(self):
        try:
            await self.project_client.agents.delete_agent(self.agent_incident.id)
            await self.project_client.agents.delete_agent(self.agent_devops.id)
        except Exception as ex:
            print(ex)
